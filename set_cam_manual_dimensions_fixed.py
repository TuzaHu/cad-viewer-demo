...
def read_dimensions_from_file():
    ...
    try:
        ...
        if png_width and png_height and template_width and template_height:
            ...
            print(f"   SVG: {template_width} x {template_height} mm")
...
# === Read manual dimensions from dimensions.txt ===
png_width, png_height, svg_width, svg_height = read_dimensions_from_file()
...
page_width = svg_width * scale_factor  # in meters
page_height = svg_height * scale_factor  # in meters

print(f"SVG dimensions: {svg_width}mm x {svg_height}mm")
print(f"Blender units: {page_width}m x {page_height}m")
...
# === Configure camera using actual SVG dimensions ===
cam.data.type = 'ORTHO'
cam.data.ortho_scale = page_width  # Use actual SVG width
cam.location = (page_width / 2, page_height / 2, 10)  # Center above the SVG area
cam.rotation_euler = (0, 0, 0)  # look straight down (Z–)

bpy.context.scene.camera = cam

# === Render output using manual PNG dimensions ===
scene = bpy.context.scene

# Use manual PNG dimensions for render resolution
scene.render.resolution_x = png_width
scene.render.resolution_y = png_height
scene.render.resolution_percentage = 100

print(f"✅ Camera perfectly aligned to SVG size: {svg_width} mm x {svg_height} mm")
...
print(f"✅ Camera now matches the actual SVG dimensions, not template dimensions!") 