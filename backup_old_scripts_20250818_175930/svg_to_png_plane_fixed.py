...
FIXED VERSION - Consistent with all other scripts
"""
...
def convert_svg_to_png(input_svg, output_png, width_mm=297, height_mm=210):
    ...
    try:
        ...
        print(f"Output dimensions: {width_pixels} x {height_pixels} pixels")
        print(f"DPI: {dpi}")
        ...
        if result.returncode == 0:
            ...
            print(f"✓ High quality PNG with {dpi} DPI")
            return True
        else:
...
'''
    ...
    with open('blender_svg_to_png_plane_fixed.py', 'w') as f:
        f.write(blender_script)
    
    print("Created blender_svg_to_png_plane_fixed.py for PNG import")

if __name__ == "__main__":
    input_svg = "exported2.svg"
    output_png = "exported2.png"
    
    print("=== SVG TO PNG PLANE CONVERTER (FIXED) ===\\n")
    ...
    if convert_svg_to_png(input_svg, output_png, template_width, template_height):
        print("\\n✓ Successfully created high-quality PNG!")
        
        # Create Blender import script
        create_blender_import_script()
        
        print("\\n=== PNG PLANE IMPORT GUIDE (FIXED) ===")
        print("1. Use exported2.png in Blender")
        print("2. Run blender_svg_to_png_plane_fixed.py for complete workflow")
        print("3. PNG will be positioned exactly like the reference plane")
        print("4. All scripts now use consistent high resolution")
